// PRIME MX Portal · Auth + recuperación de contraseña v6
(function(){
  'use strict';
  ...
  function showRecovery(){recoveryMode=true;$('#loginForm')?.classList.add('hidden');$('#recoveryForm')?.classList.remove('hidden');$('#gate')?.classList.remove('hidden');$('#app')?.classList.add('hidden');setRecoveryMsg('Elige una contraseña nueva de al menos 8 caracteres.');setTimeout(()=>$('#newPass')?.focus(),50);}
  ...
  async function enter(user){
    try{
      ...
      $('#userName').textContent=p.data.nombre||p.data.email||user.email||'';
      $('#userRole').textContent=p.data.role||'';
      renderPortal(mods.data||[]);showApp();setMsg('Acceso correcto.');return true;
    }catch(err){
      console.error('PRIME Portal enter',err);
      const m=String(err?.message||err||'No se pudo completar el acceso.');
      if(m.includes('PROFILE_TIMEOUT'))setMsg('La sesión inició, pero el perfil tardó demasiado.',true);
      else if(m.includes('MODULES_TIMEOUT'))setMsg('La sesión inició, pero los módulos tardaron demasiado.',true);
      else setMsg(m,true);
      return false;
  ...
  async function login(email,password){
    ...
    try{
      const r=await withTimeout(client.auth.signInWithPassword({email,password}),AUTH_TIMEOUT,'AUTH_TIMEOUT');
      if(r.error)throw r.error;if(!r.data?.user)throw new Error('No se recibió el usuario de la sesión.');
      await enter(r.data.user);
    }catch(err){
      console.error('PRIME Portal login',err);
      const m=String(err?.message||err||'');
      if(m.includes('Invalid login credentials'))setMsg('Correo o contraseña incorrectos.',true);
      else if(m.includes('AUTH_TIMEOUT'))setMsg('El servicio de acceso tardó demasiado. Recarga e intenta otra vez.',true);
      else setMsg(m||'No fue posible iniciar sesión.',true);
      setBusy(false);
    }
  }
  ...
  async function requestPasswordReset(){
    ...
    try{
      ...
      console.error('PRIME Portal reset request',err);
      const m=String(err?.message||err||'');
      if(m.includes('RESET_TIMEOUT'))setMsg('El servicio de recuperación tardó demasiado. Intenta nuevamente.',true);
      else setMsg('No fue posible enviar la recuperación en este momento. Intenta nuevamente.',true);
    }finally{setBusy(false);}
  }
  ...
  async function saveNewPassword(){
    ...
    try{
      const r=await withTimeout(client.auth.updateUser({password:p1}),AUTH_TIMEOUT,'UPDATE_PASSWORD_TIMEOUT');
      if(r.error)throw r.error;
      setRecoveryMsg('Contraseña actualizada correctamente. Ya puedes iniciar sesión con tu nueva contraseña.');
      await client.auth.signOut({scope:'local'}).catch(()=>{});
      history.replaceState(null,'',location.pathname);
      setTimeout(()=>{showLogin();setMsg('Contraseña actualizada. Inicia sesión con tu nueva contraseña.');$('#email')?.focus();},900);
    }catch(err){
      console.error('PRIME Portal update password',err);
      const m=String(err?.message||err||'');
      if(m.includes('UPDATE_PASSWORD_TIMEOUT'))setRecoveryMsg('El cambio tardó demasiado. Intenta nuevamente.',true);
      else setRecoveryMsg(m||'No fue posible actualizar la contraseña.',true);
  ...
  function bind(){
    ...
    const forgot=$('#forgotBtn');if(forgot)forgot.onclick=requestPasswordReset;
    const recovery=$('#recoveryForm');if(recovery)recovery.onsubmit=async e=>{e.preventDefault();await saveNewPassword();};
    const cancel=$('#cancelRecovery');if(cancel)cancel.onclick=async()=>{await client.auth.signOut({scope:'local'}).catch(()=>{});history.replaceState(null,'',location.pathname);showLogin();};
  ...
  async function boot(){
    ...
    try{
      const r=await withTimeout(client.auth.getSession(),6000,'SESSION_TIMEOUT');
      if(recoveryMode)return;
      if(r.data?.session?.user)await enter(r.data.session.user);
    }catch(err){console.warn('PRIME Portal boot',err);}
  }
